...
| `S5` | a **critical model, or an incremental one, that CODEOWNERS does not own**. Stage E says a Partner approves a critical model and "CODEOWNERS enforces this"; Control 5A says incremental models are listed explicitly. Neither was checked, so a critical model in a directory no line covers merged on the Author's approval alone, and nothing said so. `check` now reads the CODEOWNERS file the way git does — gitignore patterns, the last matching line wins, a line with no owner un-owns — and blocks when the sql or the yml of such a model is owned by nobody, or when there is no CODEOWNERS file at all |
