...
| Path | What it is |
| --- | --- |
| `slp.py` | The whole tool: three commands, thirty rules, one file you can read in one sitting. |
...
5. **Turn on branch protection** for `main`: require pull requests, require
   ...
   are the job names in `ci.yml`. Block force-push on every branch (README §2
   Control 1). Set the repository variable `AGENT_LOGIN` to the bot user of
   Control 1, so the gate is required on the pull requests it opens and advisory
   on yours.
...
lack of anything to check (`S4`); it reads one thing inside them — a `config()`
that says `materialized='incremental'`, for `S5` — never calls git, and never
...
**What CODEOWNERS owns.** Two of the framework's approval rules are not rules
about the yml at all. Stage E says a Partner approves a critical model and
"CODEOWNERS enforces this"; Control 5A says incremental models are listed
explicitly, because a selector cannot tell one from another. Both degrade
silently: a critical model in a directory no CODEOWNERS line covers merges on
the Author's approval alone, and nothing in the platform says so. `S5` reads
`.github/CODEOWNERS` (or `CODEOWNERS`, or `docs/CODEOWNERS`) the way git does
— gitignore patterns, the last matching line wins, a line with no owner
un-owns — and blocks when the sql or the yml of a critical model, or of a model
whose config says `materialized: incremental`, is owned by nobody. It cannot
say whether the owner is the *right* team; it can say whether there is one.
It reads the sql for one thing only, the `config()` that says incremental;
`+materialized` set in `dbt_project.yml` is not read.
...
which reads as a config edit rather than as a removed test. For the three rules
that need history — `G7`, `I1`, `I4` — it walks the commits with
...
**What the yml cannot show.** Every rule above reads the model yml, and dbt
lets the same weakening be written where no yml changes. A `{% test unique %}`
added under `tests/generic/` or `macros/` replaces the built-in `unique`
everywhere it is declared, because dbt resolves macros from the project before
its own; a singular test under `tests/` carries its `severity`, `enabled`,
`error_if` and the rest inside its own `{{ config() }}`. So `G9` blocks any
change on the branch to a protected path of Control 5A — `.github/`,
`.pre-commit-config.yaml`, `CODEOWNERS`, `AGENTS.md`, `dbt_project.yml`,
`macros/`, `models/semantic/`, `docs/profile/`, `tools/` — and any file added
under `tests/generic/`. Paths with a rule of their own stay with it, so one
change is one finding: the package files are `G6`, `analyses/reconciliation_*`
is `G5`, a test file already there is `G1`. `G10` reads the `config()` of every
singular test the branch adds and blocks one that cannot fail, with the same
predicate `T1` and `G3` use. CODEOWNERS still makes a human approve those
paths; the gate makes them a red check, so the approval is not the only thing
standing, and on the agent's own pull requests nobody has to notice. A human
who must change one does it in a pull request of their own — which is the next
paragraph.

**Gate scope.** Inside a pull request the gate judges every commit, whoever
wrote it: a commit author is text anyone can write, and there is nothing to
check it against. Which pull requests it *blocks* is decided by who opened
them, which is an identity the platform authenticates. `templates/ci.yml`
requires the gate on the pull requests opened by the agent identity of
Control 1 (the repository variable `AGENT_LOGIN`; unset, it is required on
every pull request, because a variable nobody set must not make a gate
optional) and runs it advisory on everyone else's, where its findings go to
the job summary and CODEOWNERS decides. What follows: in an agent PR nobody
weakens a test — not the agent, not a human. A human who must change a test, a
macro, a package pin or the CI does it **before the agent starts**, as part of
writing the spec, or in a pull request of their own, where the gate is read
and not enforced.
...
full data, produces the diff and runs `compare --base`. Both pipe their output
into the job summary, so the Author reads the findings without opening the
logs. Three things the 0.3.0 template did not do:

- **It runs the tools from the base branch's copy of `tools/`.** A pull request
  that edits the gate was judged by the edited gate: `G9` now blocks the edit,
  but the gate that says so would have been the edited one. Each job extracts
  `tools/` at the pull request's base with `git archive` and runs that copy.
  `.github/` has no such answer — a workflow runs from the pull request's own
  tree — so CODEOWNERS is what stands there, and a reviewer reads the diff of
  this file whenever a pull request touches it.
- **The gate is required on the agent's pull requests and advisory on the
  others.** The repository variable `AGENT_LOGIN` names the bot user of
  Control 1; a pull request that user opened runs `slp gate` as a required
  step, any other runs `slp gate (advisory)`, whose findings go to the job
  summary and whose exit code does not fail the job. Unset, the gate is
  required everywhere. See *Gate scope* above for why.
- **`diff` runs when the pull request is marked ready for review, and on every
  push after that.** A required check belongs to a commit, so a diff of code
  that has since changed is worth what a dismissed approval is worth. It does
  not run on a draft, which is why `AGENTS.md` tells the agent to open the pull
  request as a draft and mark it ready when Stage C is done.
...
write it — a template that is shipped unedited fails closed. Set the repository
variable `AGENT_LOGIN` to the bot user of Control 1. Finally, list `ci` and
`diff` as required checks in branch protection.
...
| §3 Stage E — "Critical model: a Partner approves. CODEOWNERS enforces this"; §2 Control 5A — "Incremental models (list explicitly)" | `S5` | `check/critical_unowned` | `check/critical_owned` |
...
| §2 Control 5A, §3 Stage C Rule 8 — "Do not edit protected paths" | `G9` | `gate/G9_generic_test_added` | `gate/G9_ok_untouched` |
| §2 Control 5B — "A test added that cannot fail", for a singular test under `tests/` | `G10` | `gate/G10_singular_born_warn` | `gate/G10_ok_singular_plain` |
...
| §3 Stage A — "the 6 fields must be read and approved by the human before any line of code is written" | `I4` | `gate/I4_spec_first_written_on_branch` | `gate/I4_ok_spec_from_main` |
...
| What a change to `dbt_project.yml` or a macro *does* | `G9` blocks any change to `dbt_project.yml` or under `macros/` on the branch, so `data_tests: {+severity: warn}` cannot land in an agent's pull request unseen. The gate still does not read either file: a human pull request that changes one is CODEOWNERS' to judge, and the gate will not be the one to say what the change does to the tests. README §2 Control 5B, Rule 3. |
...
| History-shaped evasion of `G7`, `I1` and `I4` | The three rules read the branch's history, so all three depend on its shape. The walk uses `--first-parent`, which skips work done on a side branch and merged in: a spec created *and* edited inside such a branch passes `G7`, and `I1`'s count is understated. A rewritten history is worse, and it is what agents do by habit: `commit --amend`, a rebase, a squash erase the intermediate commits outright, so for a spec first written on the branch `G7` has nothing to compare against, `I1` reports zero and `I4` points at the rewritten commit. Same content, three verdicts depending on branch topology, which sits badly with Principle 3. Nothing the gate can read tells it a history was rewritten, so the lock has to: README §2 Control 1 now asks for force-push to be blocked on every branch, which is one ruleset. With it, keep the pull request's commits and read `I1` as a floor; without it, `G7`, `I1` and `I4` are advisory. A spec that existed on `main` is safe either way, because the merge-base is what `G7` compares it to. README §3 Stage B. |
