#!/usr/bin/env python3
"""Mutation testing of the tool, with the stdlib: every mutant of slp.py is killed.
...
out) and the mutant is spliced into the copy's slp.py by source span, so the
real file is never touched and comments and quotes elsewhere stay byte for
...
IGNORED = shutil.ignore_patterns(".git", ".local", "__pycache__", ".pytest_cache", ".venv",
                                 "venv", ".coverage", ".coverage.*", "build", "dist", "*.egg-info")
...
SYMBOL = {ast.Lt: "<", ast.LtE: "<=", ast.Gt: ">", ast.GtE: ">=", ast.Eq: "==",
          ast.NotEq: "!=", ast.In: "in", ast.NotIn: "not in"}
# The fixture-driven test file of each command, by the prefix of its rules.
SELECTION = (("check_", "test_check.py"), ("gate_", "test_gate.py"),
             ("compare_", "test_compare.py"), ("cmd_", "test_cli.py"))


class Mutant:
    """One change to the source: where, what, the mutated text, and later the verdict."""

    def __init__(self, op, function, line, what, text):
        self.op, self.function, self.line, self.what, self.text = op, function, line, what, text
...
def rule_names():
    ...
    import slp
    return set(f.__name__ for f in slp.CHECK_RULES + slp.GATE_RULES + slp.COMPARE_RULES
...
def sites(tree, rules):
    """Every place one operator applies, in source order: (op, node kind, position, extra)."""
    found = []
    for node in ast.walk(tree):
        if isinstance(node, ast.Compare):
            for index, op in enumerate(node.ops):
                if type(op) in FLIP:
                    found.append(("cmp", "Compare", _pos(node), index))
        elif isinstance(node, ast.BoolOp):
            found.append(("bool", "BoolOp", _pos(node), 0))
        elif isinstance(node, ast.UnaryOp) and isinstance(node.op, ast.Not):
            found.append(("not", "UnaryOp", _pos(node), 0))
        elif isinstance(node, ast.Constant) and isinstance(node.value, bool):
            found.append(("true", "Constant", _pos(node), 0))
        elif isinstance(node, ast.Constant) and isinstance(node.value, int):
            found.append(("int", "Constant", _pos(node), "plus1"))
            if node.value in (0, 1):
                found.append(("int", "Constant", _pos(node), "swap01"))
        elif isinstance(node, ast.Call) and isinstance(node.func, ast.Name):
            if node.func.id == "block":
                found.append(("block", "Call", _pos(node), 0))
            elif node.func.id == "sorted" and node.args and enclosing(tree, node.lineno) == "report":
                found.append(("sorted", "Call", _pos(node), 0))
        elif isinstance(node, ast.FunctionDef) and node.name in rules:
            found.append(("rule", "FunctionDef", _pos(node), node.name))
...
class Apply(ast.NodeTransformer):
    ...
    def visit(self, node):
        if type(node).__name__ == self.kind and hasattr(node, "lineno") and _pos(node) == self.pos \
...
def requote(snippet):
    ...
    for token in reversed(tokens):
        text = token.string
        if token.type == tokenize.STRING and text.startswith("'") and not text.startswith("'''") \
...
def mutants(source):
    """Every mutant of the source, each compiling and each different from it, with stable ids."""
    tree, rules, out, ordinal, seen = ast.parse(source), rule_names(), [], {}, set()
    for site in sites(tree, rules):
        ...
        try:
            ...
            continue
        mutant = Mutant(site[0], enclosing(tree, site[2][0]), site[2][0], apply.what, text)
...
def run(copy, paths, timeout):
    ...
    try:
                              ...
                              + paths, cwd=str(copy), capture_output=True, text=True,
...
def judge(copy, original, mutant, timeout):
...
def campaign(chosen, jobs, log):
    ...
    with tempfile.TemporaryDirectory() as folder:
        ...
        if verdict != "survived":
            raise SystemExit("mutants: the suite is %s on the unmutated tool; fix that first"
                             % ("red" if verdict == "killed" else "timing out"))
        timeout = max(60, 5 * baseline)
        log("mutants: the suite is green in %.0fs; %d mutants, %d worker%s, timeout %.0fs"
            % (baseline, len(chosen), jobs, "" if jobs == 1 else "s", timeout))

        def work(mutant):
            copy = copies.get()
            try:
                return judge(copy, original, mutant, timeout)
            finally:
                copies.put(copy)
        with concurrent.futures.ThreadPoolExecutor(max_workers=jobs) as pool:
            for number, done in enumerate(pool.map(work, chosen), start=1):
                log("%4d/%d %-40s %-9s %5.1fs  %s" % (number, len(chosen), done.id, done.verdict,
                                                      done.seconds, done.what))


def report(chosen, listed, short):
    """The findings in the house format, the summary line, and the exit code."""
    findings, killed, equivalent, timeouts = [], 0, 0, 0
    ids = set(m.id for m in chosen)
    for mutant in chosen:
        where = (TARGET, mutant.function)
        if mutant.verdict == "timeout":
            timeouts += 1
            findings.append(("INFO",) + where + ("%s timed out, which counts as killed: %s at line %d"
                                                 % (mutant.id, mutant.what, mutant.line), "MUTANT"))
        elif mutant.verdict == "killed":
            killed += 1
            if mutant.id in listed:
                findings.append(("BLOCK",) + where + ("%s is listed as equivalent and the suite "
                                                      "kills it now; remove the line" % mutant.id, "MUTANT"))
        elif mutant.id in listed:
            equivalent += 1
            findings.append(("INFO",) + where + ("%s survived and is listed as equivalent: %s"
                                                 % (mutant.id, listed[mutant.id]), "MUTANT"))
        else:
            findings.append(("BLOCK",) + where + ("%s survived: %s at line %d, and no test tells "
                                                  "the two apart" % (mutant.id, mutant.what, mutant.line), "MUTANT"))
    for name in sorted(short):
        findings.append(("BLOCK", str(EQUIVALENTS.relative_to(ROOT)), "", "%s needs a reason of at "
                         "least four words" % name, "MUTANT"))
    for name in sorted(set(listed) - ids):
        if not any(m.id == name for m in chosen):
            findings.append(("BLOCK", str(EQUIVALENTS.relative_to(ROOT)), "", "%s is listed and no "
                             "such mutant exists; remove the line" % name, "MUTANT"))
    ...
    if blocks:
        ...
        return 1
    print("mutants: OK (%d killed, %d equivalent, %d timed out)" % (killed, equivalent, timeouts))
    return 0
...
def main(argv=None):
    ...
    chosen = [m for m in mutants(source) if not args.only or m.function == args.only]
    ...
    if args.list:
        for mutant in chosen:
            print("%-40s line %-5d %s" % (mutant.id, mutant.line, mutant.what))
