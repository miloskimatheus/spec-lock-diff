<!-- The example's copy of tools/templates/AGENTS.md, with the three lines a
     copier must resolve already resolved. In your own repository this file lives
     at the root, is a protected path, and is read by the agent - not by you. -->
...
python tools/slp.py gate --base main   # the branch this pull request targets
dbt compile
dbt test --select test_type:unit
dbt build
```

The marts of this project are in `models/marts/`, which is the default, so
neither command needs `--marts-path`. If yours are elsewhere, add
`--marts-path <dir>` to both commands, once per directory, and keep the values
...
- `.github/`, `.pre-commit-config.yaml`, `CODEOWNERS`, `AGENTS.md`,
  ...
  lists. This list is the one in `.github/CODEOWNERS`, kept identical on
  purpose. `gate` blocks a
